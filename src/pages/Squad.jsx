...
export default function Squad({ onBack }) {
  ...
  function saveEdit() {
    const updated = {
      ...data,
      squad: data.squad.map(p => p.id === editId ? { ...p, name: editName.trim(), number: editNumber.trim(), role: editRole } : p),
    }
    setData(updated)
    saveData(updated)
    setEditId(null)
  ...
  return (
    <div style={{ minHeight: '100dvh', background: s.bg, color: s.text, fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ padding: '48px 16px 16px' }}>
        <button onClick={onBack} style={{ color: s.muted, background: 'none', border: 'none', fontSize: 14, cursor: 'pointer', marginBottom: 16, display: 'block' }}>← Volver</button>
        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700 }}>Mi Equipo</h1>
        <p style={{ margin: '4px 0 0', color: s.muted, fontSize: 14 }}>La plantilla se carga automáticamente en cada partido</p>
      </div>
      ...
      <form onSubmit={e => { e.preventDefault(); addPlayer() }} style={{ padding: '0 16px 16px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
          <input
            style={{ width: 64, background: s.card, border: `1px solid ${s.border}`, color: s.text, borderRadius: 12, padding: '12px 8px', textAlign: 'center', fontSize: 18, fontWeight: 700, outline: 'none' }}
            placeholder="#"
            inputMode="numeric"
            value={newNumber}
      ...
      <div style={{ padding: '0 16px 32px' }}>
        {squad.length === 0 ? (
          ...
          <>
            <p style={{ color: s.muted, fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 12 }}>{squad.length} integrantes</p>
                  ...
                  {editId === p.id ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: s.card, borderRadius: 12, padding: '12px 16px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 16 }}>
                        <button onClick={() => startEdit(p)} style={{ color: s.muted, background: 'none', border: 'none', fontSize: 14, cursor: 'pointer' }}>Editar</button>
