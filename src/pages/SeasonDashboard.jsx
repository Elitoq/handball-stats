...
export default function SeasonDashboard({ onBack, onOpenMatch, lang = 'es' }) {
    ...
    return (
      <div style={page}>
        <div style={{ padding: '48px 16px 16px' }}>
          <button onClick={onBack} style={linkBtn}>← {t('back', lang)}</button>
  ...
  matches.forEach(m => {
    m.players.forEach(p => {
      ...
      acc.goals += s.goals; acc.misses += s.misses; acc.saves += s.saves
      acc.conceded += s.conceded; acc.exclusions += s.exclusions; acc.turnovers += s.turnovers
    ...
    <div style={page}>
      <div style={{ padding: '48px 16px 12px', position: 'sticky', top: 0, background: '#030712', zIndex: 10 }}>
        ...
        <p style={{ color: '#6b7280', fontSize: 13, margin: 0 }}>
          {matches.length} {matches.length !== 1 ? t('season.n_matches', lang) : t('season.n_match', lang)}
        </p>
      </div>

      <div style={{ padding: '0 16px 48px' }}>

        <Section title={t('season.results', lang)}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 8 }}>
            <StatCard label={t('stat.wins', lang)}   value={wins}   color="#4ade80" />
            <StatCard label={t('stat.draws', lang)}  value={draws}  color="#facc15" />
            <StatCard label={t('stat.losses', lang)} value={losses} color="#f87171" />
          </div>
          {(wins + draws + losses) > 0 && (
            <ResultBar wins={wins} draws={draws} losses={losses} />
          )}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
            <StatCard label={t('stat.goals_for', lang)} value={goalsFor}     color="#4ade80" />
            <StatCard label={t('stat.goals_ag', lang)}  value={goalsAgainst} color="#f87171" />
          </div>
        </Section>

        <Section title={t('season.team_stats', lang)}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 8 }}>
            <StatCard label={t('stat.goals', lang)}      value={totalGoals}  color="#4ade80" />
            <StatCard label={t('stat.misses', lang)}     value={totalMisses} color="#facc15" />
            <PctStatCard label={t('stat.efficiency', lang)} success={totalGoals} total={totalGoals + totalMisses} color="#7eb3ff" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 8 }}>
            <StatCard label={t('stat.saves', lang)}    value={totalSaves}   color="#60a5fa" />
            <StatCard label={t('stat.conceded', lang)} value={totalConceded} color="#f87171" />
            <PctStatCard label={t('stat.save_pct', lang)} success={totalSaves} total={totalSaves + totalConceded} color="#7eb3ff" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            <StatCard label={t('stat.exclusions', lang)} value={totalExclusions} color="#f87171" />
            <StatCard label={t('stat.turnovers', lang)}  value={totalTurnovers}  color="#fb923c" />
          </div>
        </Section>

        {trend.length > 1 && (
          <Section title={t('season.trend', lang)}>
            <GoalTrendChart matches={trend} lang={lang} />
          </Section>
        )}

        {scorers.length > 0 && (
          <Section title={t('season.scorers', lang)}>
        ...
        {goalkeepers.length > 0 && (
          <Section title={t('season.keepers', lang)}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {goalkeepers.map((p, i) => (
        ...
        <Section title={t('season.matches', lang)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[...results].sort((a, b) => b.createdAt - a.createdAt).map(m => (
              <MatchResultRow key={m.id} match={m} onOpen={() => onOpenMatch(m.id)} lang={lang} />
            ))}
          </div>
        </Section>
...
function MatchResultRow({ match, onOpen, lang }) {
  const res = match.result
  const resColor = res === 'W' ? '#4ade80' : res === 'L' ? '#f87171' : res === 'D' ? '#facc15' : '#6b7280'
  const resLabel = lang === 'en'
    ? (res === 'W' ? 'W' : res === 'L' ? 'L' : res === 'D' ? 'D' : '?')
    : (res === 'W' ? 'V' : res === 'L' ? 'D' : res === 'D' ? 'E' : '?')
  return (
    <button onClick={onOpen} style={{ width: '100%', background: '#1f2937', border: 'none', borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', color: 'white', textAlign: 'left' }}>
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <div style={{ color: '#6b7280', fontSize: 11 }}>{match.events.length} {lang === 'en' ? 'events' : 'acciones'}</div>
      </div>
    </button>
  )
}

function GoalTrendChart({ matches, lang }) {
    ...
    <div style={{ background: '#1f2937', borderRadius: 12, padding: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <span style={{ color: '#6b7280', fontSize: 11 }}>{t('season.our_goals', lang)}</span>
        </div>
      </div>
    </div>
  )
