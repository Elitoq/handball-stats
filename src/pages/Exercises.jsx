...
export default function Exercises({ lang = 'es' }) {
  const [filter, setFilter] = useState('all') // 'all' | 'court' | 'physical'
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#030712', color: 'white', fontFamily: 'system-ui, sans-serif', paddingBottom: 80 }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: active ? '#1a56db' : '#1f2937',
      ...
      <div style={{ padding: '16px 16px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {visible.length === 0 && (
          <p style={{ color: '#4b5563', fontSize: 14, textAlign: 'center', paddingTop: 32 }}>
            {t('ex.empty', lang)}
          </p>
        )}
  ...
  return (
    <div
      style={{
        background: '#111827',
        borderRadius: 16,
        border: `1px solid ${expanded ? '#1e3a7a' : '#1f2937'}`,
        overflow: 'hidden',
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          padding: '14px 16px', display: 'flex', alignItems: 'flex-start', gap: 12, textAlign: 'left',
        }}
      >
        {/* Type icon */}
        <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'white', marginBottom: 3, lineHeight: 1.2 }}>{name}</div>
      ...
      {expanded && (
        <div style={{ padding: '0 16px 16px', borderTop: '1px solid #1f2937' }}>
          <p style={{ color: '#d1d5db', fontSize: 14, lineHeight: 1.65, marginTop: 14, marginBottom: 14 }}>
            {desc}
          </p>
    ...
    <div style={{ display: 'flex', gap: 4 }}>
      {[1, 2, 3].map(i => (
        <div key={i} style={{
          ...
          background: i <= n ? colors[n] : '#374151',
        }} />
      ))}
    </div>
  )
